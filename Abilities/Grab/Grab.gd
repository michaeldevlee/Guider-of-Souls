...
onready var chain = $Chain		# A slightly easier reference to the links
...
func _process(_delta: float) -> void:
	...
	chain.position = tip_loc						# The links are moved to start at the tip
	chain.region_rect.size.y = tip_loc.length()		# and get extended for the distance between (0,0) and the tip
